...
async function gatherBrand(
  entity: EntityKey,
  opts: { stripe: boolean; ghl: boolean; qb: boolean },
): Promise<BrandData> {
  ...
  if (opts.ghl) {
    ...
    if (!m.error) parts.push(`Leads: ${m.new30d} (30d), top source ${m.topSource ?? "n/a"}`);
  }
  if (opts.qb) {
...
export async function buildBriefData(): Promise<BrandData[]> {
  const [stripeSet, ghlSet, qb] = await Promise.all([
